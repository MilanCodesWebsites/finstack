...
const FIATS = ['RMB','GHS','XAF','XOF','USD'];
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {CRYPTOS.map(c => {
                        ...
                        return (
                          ...
                          }} className={cn('p-4 rounded-lg border text-left transition relative overflow-hidden', active ? 'border-blue-600 ring-2 ring-blue-200 bg-gradient-to-br from-blue-50 to-white' : 'hover:bg-gray-50')}> 
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {FIATS.map(f => {
                        ...
                        return (
                          <button key={f} disabled={disabled} onClick={() => update('pair', newPair)} className={cn('p-3 rounded-lg border text-center text-sm font-medium transition relative', active ? 'border-green-600 ring-2 ring-green-200 bg-gradient-to-br from-green-50 to-white' : disabled ? 'opacity-30 cursor-not-allowed' : 'hover:bg-gray-50')}> 
                            {f}
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      {ad.paymentMethods.map(m => (
                        <span key={m} className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs flex items-center gap-1">
                          {m}
                          {(['Bank Transfer','Mobile Money'].includes(m)) ? null : (
...
function ReviewRow({ label, value }: { label: string; value: string; }) {
  return (
    <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-4 py-2 border-b last:border-0 border-gray-100">
      ...
      <span className="font-medium text-gray-800 break-words text-sm">{value}</span>
    </div>
  );
}
